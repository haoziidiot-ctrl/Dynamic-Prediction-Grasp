...
消融实验2：random_mpc（约束+补偿链路）vs default_mpc（解析链路）

实现映射（DPG_mujoco_final 框架）：
- random_mpc_on = 1 -> use_constrained_qp=True
  且启用 ee x/y 上界约束（与 DPG_main.py 默认 constrained 配置一致）
- random_mpc_on = 0 -> use_constrained_qp=False
  走默认解析链路

其余参数固定一致，使用配对随机回合做公平对照。
"""
...
def _build_controller(
    ...
    *,
    ...
    # 配置与 DPG_mujoco_final/DPG_main.py 对齐，只切 random_mpc 开关。
        ...
        json.dump(
            {
                ...
                "switch_mapping": {
                    "random_mpc_on": {
                        ...
                        "qp_enforce_ee_y_upper": True,
                        "qp_ee_y_margin": 0.0,
                        "note": "constrained QP path + drift compensation in controller",
                    },
                    "random_mpc_off": {
                        "use_constrained_qp": False,
                        "note": "default analytic path",
                    },
                },
                "fixed_switches": {
                    "use_uncertainty_aware_weighting": bool(uncertainty_on),
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="output directory (default: DPG_mujoco_final/experiment_results/ablation_random_mpc_<timestamp>)",
    )
