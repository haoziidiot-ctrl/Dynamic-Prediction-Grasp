...
消融实验1：自适应权重（uncertainty-aware weighting）开/关对比。

统计口径与之前版本保持一致：
- 成功率
- zone（禁区）时间与比例
- target/hold/attach 误差
- 高频差分指标（target_err / attach_err / qdot）

公平性：同一 episode 的随机参数固定，ON/OFF 各跑一次。
"""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="output directory (default: DPG_mujoco_analytic/experiment_results/ablation_uncertainty_<timestamp>)",
    )
...
def main():
    ...
    if args.out_dir is None:
        ...
        out_dir = Path(__file__).resolve().parent / "experiment_results" / f"ablation_uncertainty_{stamp}"
    else:
