...
class MPCController:
    ...
    def __init__(
        ...
        qp_enable_during_hold: bool = False,
        qp_activate_x_err_threshold: float = 0.12,
        # random_mpc: 以“减少禁区时间”为主目标的约束门控与输出补偿
        qp_zone_avoid_enable: bool = False,
        qp_zone_x_half_width: float = 0.05,
        qp_zone_y_back: float = 0.10,
        qp_zone_y_margin: float = 0.005,
        qp_zone_x_gate_margin: float = 0.01,
        qp_zone_escape_gain_x: float = 1.6,
        qp_zone_escape_gain_y: float = 1.2,
        qp_zone_escape_speed_cap: float = 0.28,
        qp_zone_escape_disable_near_err: float = 0.05,
        qp_zone_escape_full_err: float = 0.16,
    ):
        ...
        self.qp_enable_during_hold = bool(qp_enable_during_hold)
        self.qp_activate_x_err_threshold = max(0.0, float(qp_activate_x_err_threshold))
        self.qp_zone_avoid_enable = bool(qp_zone_avoid_enable)
        self.qp_zone_x_half_width = max(1e-4, float(qp_zone_x_half_width))
        self.qp_zone_y_back = max(1e-4, float(qp_zone_y_back))
        self.qp_zone_y_margin = max(0.0, float(qp_zone_y_margin))
        self.qp_zone_x_gate_margin = max(0.0, float(qp_zone_x_gate_margin))
        self.qp_zone_escape_gain_x = max(0.0, float(qp_zone_escape_gain_x))
        self.qp_zone_escape_gain_y = max(0.0, float(qp_zone_escape_gain_y))
        self.qp_zone_escape_speed_cap = max(0.0, float(qp_zone_escape_speed_cap))
        self.qp_zone_escape_disable_near_err = max(0.0, float(qp_zone_escape_disable_near_err))
        self.qp_zone_escape_full_err = max(
            self.qp_zone_escape_disable_near_err + 1e-6,
            float(qp_zone_escape_full_err),
        )
        ...
        if self.qp_zone_avoid_enable:
            # 随机MPC需要在切入抓取窗后快速穿越，避免因过强降速导致禁区停留和抓取失败
            self.qp_near_err = 0.05
            self.qp_stop_err = 0.012
            self.qp_min_qdot_scale = 0.35
    ...
    def _qp_ee_y_upper_sequence(
        ...
        *,
        ...
        if target_world_now is not None:
            tgt_now = np.asarray(target_world_now, dtype=float).reshape(3)
            y_now = float(tgt_now[1]) - margin
            if self.qp_zone_avoid_enable and current_pos_world is not None:
                ee_now = np.asarray(current_pos_world, dtype=float).reshape(3)
                x_err = float(ee_now[0] - tgt_now[0])
                x_gate = float(self.qp_zone_x_half_width + self.qp_zone_x_gate_margin)
                if abs(x_err) > x_gate:
                    y_cap = float(tgt_now[1] - self.qp_zone_y_back - self.qp_zone_y_margin)
                    y_now = min(y_now, y_cap)
            return np.full(self.mpc.horizon, y_now, dtype=float)
        future_world = self._world_future_targets(sim_time)
        y_upper = np.asarray(future_world[:, 1], dtype=float).reshape(self.mpc.horizon) - margin
        if self.qp_zone_avoid_enable and current_pos_world is not None:
            ee_now = np.asarray(current_pos_world, dtype=float).reshape(3)
            x_err = float(ee_now[0] - float(future_world[0, 0]))
            x_gate = float(self.qp_zone_x_half_width + self.qp_zone_x_gate_margin)
            if abs(x_err) > x_gate:
                y_cap = np.asarray(
                    future_world[:, 1] - self.qp_zone_y_back - self.qp_zone_y_margin,
                    dtype=float,
                ).reshape(self.mpc.horizon)
                y_upper = np.minimum(y_upper, y_cap)
        return y_upper
    ...
    def _qp_active_now(
        ...
        *,
        ...
        target_world_now: np.ndarray,
    ) -> bool:
        if not self.use_constrained_qp:
            return False
        if (not self.qp_enable_during_hold) and self.use_offset_tracking and self.offset_active:
            return False
        if self.qp_activate_x_err_threshold > 1e-9:
            ee = np.asarray(current_pos_world, dtype=float).reshape(3)
            tgt = np.asarray(target_world_now, dtype=float).reshape(3)
            if abs(float(ee[0] - tgt[0])) > self.qp_activate_x_err_threshold:
                return False
        return True

    def _qp_zone_escape_twist(
        self,
        *,
        current_pos_world: np.ndarray,
        target_world_now: np.ndarray,
    ) -> np.ndarray:
        """随机MPC输出补偿：若进入禁区，优先快速离开禁区。"""
        if (not self.use_constrained_qp) or (not self.qp_zone_avoid_enable):
            return np.zeros(3, dtype=float)
        if self.qp_zone_escape_speed_cap <= 1e-9:
            return np.zeros(3, dtype=float)

        ee = np.asarray(current_pos_world, dtype=float).reshape(3)
        tgt = np.asarray(target_world_now, dtype=float).reshape(3)
        target_err = float(np.linalg.norm(ee - tgt))
        if target_err <= self.qp_zone_escape_disable_near_err:
            return np.zeros(3, dtype=float)
        ramp = (
            (target_err - self.qp_zone_escape_disable_near_err)
            / max(self.qp_zone_escape_full_err - self.qp_zone_escape_disable_near_err, 1e-9)
        )
        escape_scale = float(np.clip(ramp, 0.0, 1.0))

        x_err = float(ee[0] - tgt[0])
        y_low = float(tgt[1] - self.qp_zone_y_back)
        y_high = float(tgt[1])
        in_y_window = (float(ee[1]) >= y_low) and (float(ee[1]) <= y_high)
        outside_x_band = abs(x_err) > float(self.qp_zone_x_half_width)
        if (not in_y_window) or (not outside_x_band):
            return np.zeros(3, dtype=float)

        ff = np.zeros(3, dtype=float)
        if x_err < -self.qp_zone_x_half_width:
            x_gap = float(-self.qp_zone_x_half_width - x_err)
            ff[0] = self.qp_zone_escape_gain_x * x_gap
        elif x_err > self.qp_zone_x_half_width:
            x_gap = float(x_err - self.qp_zone_x_half_width)
            ff[0] = -self.qp_zone_escape_gain_x * x_gap

        # 同时给一个向下 y 逃逸分量，快速离开 [y_t-0.1, y_t] 窗口
        y_exit_target = y_low - self.qp_zone_y_margin
        y_gap = float(ee[1] - y_exit_target)
        if y_gap > 0.0:
            ff[1] = -self.qp_zone_escape_gain_y * y_gap

        ff = ff * escape_scale
        norm = float(np.linalg.norm(ff))
        if norm > self.qp_zone_escape_speed_cap and norm > 1e-12:
            ff = ff * (self.qp_zone_escape_speed_cap / norm)
        return ff
    ...
    def _warm_start_to_pose(
        ...
        for k in range(steps):
            ...
            if k % self.control_every == 0:
                ...
                qp_active_now = self._qp_active_now(
                    current_pos_world=current_pos_world,
                    ...
                    callback(
                        {
                            ...
                            "qp_enabled": bool(qp_active_now),
                            "qp_ok": bool(self.mpc.last_solve_ok) if qp_active_now else True,
                            "qp_status": str(self.mpc.last_solve_status) if qp_active_now else "disabled",
                        }
                    )
