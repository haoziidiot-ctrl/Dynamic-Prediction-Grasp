...
if __name__ == "__main__":
    ...
    # 仅在接近抓取窗口时启用QP，避免全程约束拖慢并拉长禁区停留
    QP_ENABLE_DURING_HOLD = False
    QP_ACTIVATE_X_ERR_THRESHOLD = 0.12
    # random_mpc: 以“禁区时间最小化”为主目标的门控+补偿
    QP_ZONE_AVOID_ENABLE = True
    QP_ZONE_X_HALF_WIDTH = 0.05
    QP_ZONE_Y_BACK = 0.10
    QP_ZONE_Y_MARGIN = 0.005
    QP_ZONE_X_GATE_MARGIN = 0.01
    QP_ZONE_ESCAPE_GAIN_X = 1.2
    QP_ZONE_ESCAPE_GAIN_Y = 0.9
    QP_ZONE_ESCAPE_SPEED_CAP = 0.20
    QP_ZONE_ESCAPE_DISABLE_NEAR_ERR = 0.05
    QP_ZONE_ESCAPE_FULL_ERR = 0.16
    ...
    MPCController(
        ...
        qp_enable_during_hold=QP_ENABLE_DURING_HOLD,
        qp_activate_x_err_threshold=QP_ACTIVATE_X_ERR_THRESHOLD,
        qp_zone_avoid_enable=QP_ZONE_AVOID_ENABLE,
        qp_zone_x_half_width=QP_ZONE_X_HALF_WIDTH,
        qp_zone_y_back=QP_ZONE_Y_BACK,
        qp_zone_y_margin=QP_ZONE_Y_MARGIN,
        qp_zone_x_gate_margin=QP_ZONE_X_GATE_MARGIN,
        qp_zone_escape_gain_x=QP_ZONE_ESCAPE_GAIN_X,
        qp_zone_escape_gain_y=QP_ZONE_ESCAPE_GAIN_Y,
        qp_zone_escape_speed_cap=QP_ZONE_ESCAPE_SPEED_CAP,
        qp_zone_escape_disable_near_err=QP_ZONE_ESCAPE_DISABLE_NEAR_ERR,
        qp_zone_escape_full_err=QP_ZONE_ESCAPE_FULL_ERR,
    ).run()
