...
class TaskSpaceMPC:
    ...
    def solve(
        ...
        *,
        ...
        soft_upper_mask: Optional[np.ndarray] = None,
        soft_upper_penalty: float = 0.0,
        qp_solver: str = "osqp",
    ) -> np.ndarray:
        ...
        orig_dim = H.shape[0]
        rhs_aug = rhs
        if soft_upper_mask is not None:
            soft_upper_mask = np.asarray(soft_upper_mask, dtype=bool).reshape(-1)
            if soft_upper_mask.shape[0] != c_mat.shape[0]:
                raise ValueError("soft_upper_mask 长度与约束行数不一致")
            soft_rows = np.flatnonzero(soft_upper_mask)
            if soft_rows.size > 0:
                if float(soft_upper_penalty) <= 0.0:
                    raise ValueError("启用软约束时 soft_upper_penalty 必须为正")
                n_soft = int(soft_rows.size)
                slack_map = np.zeros((c_mat.shape[0], n_soft), dtype=float)
                slack_map[soft_rows, np.arange(n_soft)] = -1.0
                c_mat = np.hstack([c_mat, slack_map])
                slack_eye = np.hstack(
                    [np.zeros((n_soft, orig_dim), dtype=float), np.eye(n_soft, dtype=float)]
                )
                c_mat = np.vstack([c_mat, slack_eye])
                c_low = np.concatenate([c_low, np.zeros(n_soft, dtype=float)])
                c_up = np.concatenate([c_up, np.full(n_soft, np.inf, dtype=float)])
                H = sp.block_diag(
                    (sp.csc_matrix(H), sp.eye(n_soft, format="csc") * float(soft_upper_penalty)),
                    format="csc",
                )
                rhs_aug = np.concatenate([rhs, np.zeros(n_soft, dtype=float)])
            else:
                H = sp.csc_matrix(H)
        else:
            H = sp.csc_matrix(H)
        ...
        p = 0.5 * (H + H.T)
        n = p.shape[0]
        p = p + 1e-9 * sp.eye(n, format="csc")
        q = -rhs_aug
        p_csc = sp.triu(p, format="csc")
        ...
        self.full_solution = sol[:orig_dim]
        twist = self.full_solution[: self.task_dim]
        self.prev_twist = twist
        return twist
...
class MPCController:
    ...
    def __init__(
        ...
        use_stochastic_mpc: bool = False,
        stochastic_risk_alpha: float = 0.05,
        stochastic_sigma_scale: float = 1.0,
        stochastic_backoff_max: float = 0.08,
        stochastic_release_slowdown: float = 0.0,
        stochastic_ff_suppression: float = 0.0,
        stochastic_attach_pos_boost: float = 0.0,
        stochastic_attach_x_boost: float = 0.0,
        stochastic_attach_rot_boost: float = 0.0,
        stochastic_use_tube_feedback: bool = True,
        stochastic_tube_gain_xy: float = 0.35,
        stochastic_tube_gain_z: float = 0.20,
        stochastic_tube_max_linear: float = 0.25,
        stochastic_soft_constraint_penalty: float = 2.0e4,
    ):
        ...
        self.use_stochastic_mpc = bool(use_stochastic_mpc)
        alpha = float(np.clip(stochastic_risk_alpha, 1e-6, 0.499999))
        self.stochastic_risk_alpha = alpha
        self.stochastic_risk_beta = float(norm.ppf(1.0 - alpha))
        self.stochastic_sigma_scale = max(0.0, float(stochastic_sigma_scale))
        self.stochastic_backoff_max = max(0.0, float(stochastic_backoff_max))
        self.stochastic_release_slowdown = max(0.0, float(stochastic_release_slowdown))
        self.stochastic_ff_suppression = max(0.0, float(stochastic_ff_suppression))
        self.stochastic_attach_pos_boost = max(0.0, float(stochastic_attach_pos_boost))
        self.stochastic_attach_x_boost = max(0.0, float(stochastic_attach_x_boost))
        self.stochastic_attach_rot_boost = max(0.0, float(stochastic_attach_rot_boost))
        self.stochastic_use_tube_feedback = bool(stochastic_use_tube_feedback)
        self.stochastic_tube_gain_xy = max(0.0, float(stochastic_tube_gain_xy))
        self.stochastic_tube_gain_z = max(0.0, float(stochastic_tube_gain_z))
        self.stochastic_tube_max_linear = max(0.0, float(stochastic_tube_max_linear))
        self.stochastic_soft_constraint_penalty = max(
            1e-6, float(stochastic_soft_constraint_penalty)
        )
        self._stochastic_cov_mode = "disabled"
        self._stochastic_last_backoff_x = np.zeros(self.mpc.horizon, dtype=float)
        self._stochastic_last_backoff_y = np.zeros(self.mpc.horizon, dtype=float)
        self._stochastic_last_tube_feedback = np.zeros(3, dtype=float)
    ...
    def _reset_episode_metrics(self) -> None:
        self._episode_attach_time = np.nan
        self._episode_best_target_err = np.inf
        self._episode_qp_infeasible_count = 0
        self._episode_max_backoff_y = 0.0
        self._episode_cov_mode = "disabled"
        self._episode_grasp_time = np.nan
    ...
    def _task_error(
        ...
        if self.use_offset_tracking:
            ...
            x_error_gain *= self._stochastic_attach_ff_scale(abs_time)
            confidence = self._stochastic_attach_confidence(abs_time)
            boost = alpha * confidence
            orientation_gain *= 1.0 + self.stochastic_attach_rot_boost * boost
            x_error_gain *= 1.0 + self.stochastic_attach_x_boost * boost
        else:
        ...
        *,
        ...
        if self.qp_enforce_joint_pos or self.qp_enforce_joint_vel:
            ...
            if self.qp_enforce_joint_vel:
                ...
                l_blocks.append(-vel)
                u_blocks.append(vel)
                soft_upper_blocks.append(np.zeros_like(vel, dtype=bool))
            ...
            if self.qp_enforce_joint_pos:
                ...
                soft_upper_blocks.append(np.zeros_like(l_qpos, dtype=bool))
        ...
        if self.qp_enforce_ee_x_upper:
            ...
            soft_upper_blocks.append(
                np.full(self.mpc.horizon, bool(self.use_stochastic_mpc), dtype=bool)
            )
        ...
        return (
            ...
            np.concatenate(soft_upper_blocks),
        )
    ...
    def _qp_ee_x_upper_sequence(
        ...
        *,
        ...
        backoff = np.zeros(self.mpc.horizon, dtype=float)
        self._stochastic_last_backoff_x = backoff.copy()
        if target_world_now is not None:
            x_now = float(np.asarray(target_world_now, dtype=float).reshape(3)[0]) + margin
            return np.full(self.mpc.horizon, x_now, dtype=float) - backoff
        future_world = self._world_future_targets(sim_time)
        return (
            np.asarray(future_world[:, 0], dtype=float).reshape(self.mpc.horizon) + margin - backoff
        )
    ...
    def _qp_ee_y_upper_sequence(
        ...
        *,
        ...
        abs_time: float,
        target_world_now: Optional[np.ndarray] = None,
        future_targets_world: Optional[np.ndarray] = None,
    ) -> np.ndarray:
        margin = float(self.qp_ee_y_margin)
        backoff = self._stochastic_backoff_sequence(
            sim_time=sim_time, abs_time=abs_time, axis="y"
        )
        if self.use_offset_tracking:
            if self.offset_active:
                backoff = backoff
            else:
                backoff = np.zeros_like(backoff)
        if future_targets_world is not None:
            future_targets_world = np.asarray(future_targets_world, dtype=float).reshape(
                self.mpc.horizon, 3
            )
            return (
                np.asarray(future_targets_world[:, 1], dtype=float).reshape(self.mpc.horizon)
                - margin
                - backoff
            )
        if target_world_now is not None:
            y_now = float(np.asarray(target_world_now, dtype=float).reshape(3)[1]) - margin
            return np.full(self.mpc.horizon, y_now, dtype=float) - backoff
        future_world = self._world_future_targets(sim_time)
        return (
            np.asarray(future_world[:, 1], dtype=float).reshape(self.mpc.horizon) - margin - backoff
        )
    ...
    def _project_qdot_to_ee_y_upper(
        self,
        qdot_cmd: np.ndarray,
        *,
        jac_y_row: np.ndarray,
        current_y: float,
        y_upper: float,
    ) -> np.ndarray:
        if not self.qp_enforce_ee_y_upper:
            return qdot_cmd
        return self._project_qdot_to_axis_upper(
            qdot_cmd,
            jac_axis_row=jac_y_row,
            current_value=current_y,
            upper_value=y_upper,
        )

    def _future_covariances_xy(
        self, *, sim_time: float, abs_time: float
    ) -> Optional[np.ndarray]:
        if (not self.is_ball_rel) or (self.base_traj_for_rel is None):
            self._stochastic_cov_mode = "none"
            return None
        cov_fn = None
        if self.use_stochastic_mpc:
            cov_fn = getattr(self.base_traj_for_rel, "future_covariances_xy_closed_loop", None)
            if callable(cov_fn):
                self._stochastic_cov_mode = "closed_loop"
            else:
                cov_fn = None
        if cov_fn is None:
            cov_fn = getattr(self.base_traj_for_rel, "future_covariances_xy", None)
            self._stochastic_cov_mode = "open_loop" if callable(cov_fn) else "none"
        if not callable(cov_fn):
            return None
        # stochastic 关闭时回退到原基础 MPC 口径，避免改变协方差查询的时间基准。
        traj_time = self._ball_rel_traj_time(sim_time, abs_time) if self.use_stochastic_mpc else float(abs_time)
        self._sync_base_traj(traj_time)
        covs = np.asarray(cov_fn(self.mpc.horizon, self.control_dt), dtype=float)
        if covs.shape != (self.mpc.horizon, 2, 2):
            return None
        return covs

    def _stochastic_backoff_sequence(
        self, *, sim_time: float, abs_time: float, axis: str
    ) -> np.ndarray:
        out = np.zeros(self.mpc.horizon, dtype=float)
        if not self.use_stochastic_mpc:
            return out
        covs = self._future_covariances_xy(sim_time=sim_time, abs_time=abs_time)
        if covs is None:
            return out
        if axis == "x":
            axis_vec = np.array([1.0, 0.0], dtype=float)
        elif axis == "y":
            axis_vec = np.array([0.0, 1.0], dtype=float)
        else:
            raise ValueError("axis 仅支持 x / y")
        var = np.einsum("i,hij,j->h", axis_vec, covs, axis_vec)
        sigma = np.sqrt(np.clip(var, 0.0, None))
        out = self.stochastic_risk_beta * self.stochastic_sigma_scale * sigma
        if self.stochastic_backoff_max > 1e-12:
            out = np.clip(out, 0.0, self.stochastic_backoff_max)
        if axis == "x":
            self._stochastic_last_backoff_x = out.copy()
        else:
            self._stochastic_last_backoff_y = out.copy()
        return out

    def _stochastic_tube_feedback_twist(
        self, *, current_pos_world: np.ndarray, desired_pos_world: np.ndarray
    ) -> np.ndarray:
        if (not self.use_stochastic_mpc) or (not self.stochastic_use_tube_feedback):
            self._stochastic_last_tube_feedback[:] = 0.0
            return np.zeros(self.mpc.task_dim if self.use_orientation_task else 3, dtype=float)
        err = np.asarray(desired_pos_world, dtype=float).reshape(3) - np.asarray(
            current_pos_world, dtype=float
        ).reshape(3)
        phase_scale = 0.4 if (self.use_offset_tracking and self.offset_active) else 1.0
        corr_xyz = np.array(
            [
                phase_scale * self.stochastic_tube_gain_xy * err[0],
                phase_scale * self.stochastic_tube_gain_xy * err[1],
                phase_scale * self.stochastic_tube_gain_z * err[2],
            ],
            dtype=float,
        )
        norm_xyz = float(np.linalg.norm(corr_xyz))
        if self.stochastic_tube_max_linear > 1e-12 and norm_xyz > self.stochastic_tube_max_linear:
            corr_xyz *= self.stochastic_tube_max_linear / norm_xyz
        self._stochastic_last_tube_feedback = corr_xyz.copy()
        if self.use_orientation_task:
            twist = np.zeros(self.mpc.task_dim, dtype=float)
            twist[:3] = corr_xyz
            return twist
        return corr_xyz

    def _stochastic_attach_ff_scale(self, abs_time: float) -> float:
        if (not self.use_stochastic_mpc) or self.stochastic_ff_suppression <= 1e-9:
            return 1.0
        sim_time = max(0.0, float(abs_time) - float(self.time_offset))
        backoff = self._stochastic_backoff_sequence(sim_time=sim_time, abs_time=abs_time, axis="y")
        if backoff.size == 0:
            return 1.0
        return float(1.0 / (1.0 + self.stochastic_ff_suppression * float(backoff[0])))

    def _stochastic_attach_confidence(self, abs_time: float) -> float:
        if (not self.use_stochastic_mpc) or self.stochastic_backoff_max <= 1e-9:
            return 1.0
        sim_time = max(0.0, float(abs_time) - float(self.time_offset))
        backoff = self._stochastic_backoff_sequence(sim_time=sim_time, abs_time=abs_time, axis="y")
        if backoff.size == 0:
            return 1.0
        uncertainty = float(np.clip(backoff[0] / max(self.stochastic_backoff_max, 1e-9), 0.0, 1.0))
        return float(1.0 - uncertainty)
    ...
    def _offset_release_progress(self, abs_time: float) -> float:
        ...
        if self.offset_active:
            return 0.0
        release_time = float(self.offset_release_time_s)
        if self.use_stochastic_mpc and self.stochastic_release_slowdown > 1e-9:
            confidence = self._stochastic_attach_confidence(abs_time)
            uncertainty_ratio = float(np.clip(1.0 - confidence, 0.0, 1.0))
            release_time = release_time * (1.0 + self.stochastic_release_slowdown * uncertainty_ratio)
        if release_time <= 1e-9:
            return 1.0
        if not np.isfinite(self._offset_release_start_time):
            return 1.0
        alpha = (float(abs_time) - float(self._offset_release_start_time)) / release_time
    ...
    def _future_with_base_drift_comp(
        ...
        if phase_gain <= 1e-9:
            return future_world
        ff_scale = self._stochastic_attach_ff_scale(abs_time)
        self._sync_base_traj(abs_time)
        ...
        return np.asarray(future_world, dtype=float) - (
            phase_gain * ff_scale * self.base_ff_gain
        ) * drift

    def _future_pos_weight_scales(self, *, sim_time: float, abs_time: float) -> Optional[np.ndarray]:
        if self.use_offset_tracking:
            alpha = self._offset_release_progress(abs_time)
            phase_scale = float(
                (1.0 - alpha) * self.hold_pos_weight_scale + alpha * self.attach_pos_weight_scale
            )
            phase_scale *= 1.0 + self.stochastic_attach_pos_boost * alpha * self._stochastic_attach_confidence(abs_time)
        else:
            phase_scale = 1.0
    ...
    def _warm_start_to_pose(
        ...
        for k in range(steps):
            ...
            if k % self.control_every == 0:
                ...
                pos_weight_scales = self._future_pos_weight_scales(
                    sim_time=sim_time, abs_time=abs_time
                )
                ...
                if self.use_constrained_qp:
                    ...
                    ee_y_upper_seq = self._qp_ee_y_upper_sequence(
                        sim_time=sim_time,
                        abs_time=abs_time,
                        target_world_now=future_targets[0],
                        future_targets_world=future_targets,
                    )
                    a_cons, l_cons, u_cons, soft_mask = self._build_constrained_qp_bounds(
                        jac_pinv=jac_pinv,
                        q_ref=q_now,
                        current_pos_world=current_pos_world,
                    ...
                    twist = self.mpc.solve(
                        ...
                        soft_upper_mask=soft_mask,
                        soft_upper_penalty=self.stochastic_soft_constraint_penalty,
                    ...
                    if self.mpc.last_solve_ok:
                        ...
                        if self.use_stochastic_mpc and self.stochastic_use_tube_feedback:
                            tube_twist = self._stochastic_tube_feedback_twist(
                                current_pos_world=current_pos_world,
                                desired_pos_world=future_for_opt[0],
                            )
                            twist_cmd = twist_cmd + tube_twist
                            twist_cmd, _ = self._limit_twist_cmd(twist_cmd, twist_cmd.copy())
                        else:
                            self._stochastic_last_tube_feedback[:] = 0.0
                        if self.use_orientation_task:
                        ...
                        qdot_cmd = self._project_qdot_to_ee_y_upper(
                            qdot_cmd,
                            jac_y_row=jac_full[1],
                            current_y=float(current_pos_world[1]),
                            y_upper=float(ee_y_upper_seq[0]),
                        )
                        ...
                        if self.qp_infeasible_policy == "hold":
                            self._episode_qp_infeasible_count += 1
                    ...
                    qdot_cmd = self._project_qdot_to_ee_y_upper(
                        ...
                        jac_y_row=jac_full[1],
                        current_y=float(current_pos_world[1]),
                        y_upper=float(future_targets[0][1]),
                    )
    ...
    def _control_loop(
        ...
        while True:
            ...
            if step_count % self.control_every == 0:
                ...
                pos_weight_scales = self._future_pos_weight_scales(
                    sim_time=sim_time, abs_time=float(self.data.time)
                )
                manip_rhs = self._manipulability_guidance_rhs()
                jac_full = self._task_jacobian()
                jac = jac_full if self.use_orientation_task else jac_full[:3]
                ...
                if self.use_constrained_qp:
                    ...
                    ee_y_upper_seq = self._qp_ee_y_upper_sequence(
                        sim_time=sim_time,
                        abs_time=float(self.data.time),
                        target_world_now=future_world[0],
                        future_targets_world=future_world,
                    )
                    a_cons, l_cons, u_cons, soft_mask = self._build_constrained_qp_bounds(
                        jac_pinv=jac_pinv,
                        q_ref=q_now,
                    ...
                    if enable_base_ff:
                        ...
                        ff_stochastic_gain = self._stochastic_attach_ff_scale(self.data.time)
                    ...
                    qdot_cmd = self._project_qdot_to_ee_y_upper(
                        ...
                        jac_y_row=jac_full[1],
                        current_y=float(current_pos_world[1]),
                        y_upper=float(future_world[0][1]),
                    )
                    self.last_qdot_cmd = qdot_cmd.copy()
                    joint_bias = self.drift_gain * joint_offset * self.control_dt
                    q_des = q_des + qdot_cmd * self.control_dt + joint_bias
                    ...
                    callback(
                        {
                            ...
                            "stochastic_enabled": bool(self.use_stochastic_mpc),
                            "stochastic_beta": float(self.stochastic_risk_beta),
                            "stochastic_cov_mode": str(self._stochastic_cov_mode),
                            "stochastic_backoff_x_first": float(self._stochastic_last_backoff_x[0]),
                            "stochastic_backoff_y_first": float(self._stochastic_last_backoff_y[0]),
                            "stochastic_tube_feedback_norm": float(
                                np.linalg.norm(self._stochastic_last_tube_feedback)
                            ),
                        }
                    )
            ...
            if phase_now == "attach" and not np.isfinite(self._episode_attach_time):
                self._episode_attach_time = float(sim_time_now)
            self._episode_best_target_err = min(self._episode_best_target_err, target_err_now)
            self._episode_max_backoff_y = max(
                self._episode_max_backoff_y,
                float(self._stochastic_last_backoff_y[0]) if self._stochastic_last_backoff_y.size else 0.0,
            )
            self._episode_cov_mode = str(self._stochastic_cov_mode)
            hold_ref_now = None
            hold_err_now = np.nan
            ...
            if self.enable_grasp and (not self.grasped):
                ...
                if hold_ok:
                    self.grasped = True
                    self._episode_grasp_time = float(sim_time_now)
    ...
    def run_headless(
        ...
        *,
        ...
        return {
            "grasped": bool(self.grasped),
            "sim_time": sim_time_now,
            "grasp_time": float(self._episode_grasp_time),
            "attach_time": float(self._episode_attach_time),
            "best_target_err": float(self._episode_best_target_err),
            "qp_infeasible_count": int(self._episode_qp_infeasible_count),
            "max_backoff_y": float(self._episode_max_backoff_y),
            "stochastic_cov_mode": str(self._episode_cov_mode),
