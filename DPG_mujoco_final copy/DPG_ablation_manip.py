"""
DPG_ablation_manip.py

消融实验2：操作度梯度引导（manipulability guidance）开/关对比
...
def _build_controller(
    ...
    *,
    ...
    return MPCController(
        ...
        uncertainty_ema=0.2,
        use_manipulability_guidance=bool(manip_on),
...
def _safe_p10(values: List[float]) -> float:
    if len(values) == 0:
        return float("nan")
    return float(np.percentile(np.asarray(values, dtype=float), 10.0))
...
def _to_log10_cond(cond_val: float) -> float:
    if not np.isfinite(cond_val):
        return float("nan")
    c = float(np.clip(cond_val, 1.0, 1e12))
    return float(np.log10(c))
...
def _summary_for_mode(rows: List[Dict], manip_on: int) -> Dict:
    sub = [r for r in rows if int(r["manip_on"]) == int(manip_on)]
    n = len(sub)
    return {
        ...
        json.dump(
            {
                ...
                "ablation": "manipulability_guidance on/off",
                "fixed_switches": {
                    "use_uncertainty_aware_weighting": bool(uncertainty_on),
                },
                "random_ranges": {
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Ablation: manipulability guidance ON vs OFF under paired random episodes"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="output directory (default: DPG_mujoco_final/experiment_results/ablation_manip_<timestamp>)",
    )
...
def main():
    ...
    if args.out_dir is None:
        ...
        out_dir = Path(__file__).resolve().parent / "experiment_results" / f"ablation_manip_{stamp}"
    else:
