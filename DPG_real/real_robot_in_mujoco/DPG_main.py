...
# ================= 用户配置区（优先改这里） =================
# 轨迹类型: "ball" / "ball_in_robot"
OBJECT_TRACK = "ball_in_robot"
# real: 底盘输入来源（True=外部实时输入；False=仿真内置轨迹）
USE_REAL_BASE = True
# real: 目标点请在 DPG_real/get_robot_status.py 的 SIM_TARGET_POS_REAL 中设置
# ==========================================================
    ...
    if name == "ball_in_robot":
        if use_real_base:
            ...
            base_traj = RealTimeBaseTrajectory(
                poll_period_s=float(kf_cfg.get("poll_period_s", 0.01)),
                sigma_a=float(kf_cfg.get("sigma_a", 0.08)),
                meas_noise=float(kf_cfg.get("meas_noise", 0.005)),
            )
            ball_world = np.asarray(udp.target_world_in_sim(base_traj.sim_origin), dtype=float).reshape(3)
            return get_ball_in_robot_trajectory(
                base_traj=base_traj,
                ball_world=ball_world,
                use_kf_wrapper=False,
            )
        return get_ball_in_robot_trajectory(
            kf_cfg=kf_cfg,
            use_kf_wrapper=True,
