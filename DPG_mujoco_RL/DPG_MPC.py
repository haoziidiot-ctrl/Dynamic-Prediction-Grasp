...
封装 TaskSpace MPC 与机械臂控制逻辑：
    - TaskSpaceMPC: 无约束 QP（允许插入 RL 终端代价）。
...
class TaskSpaceMPC:
    ...
    def __init__(
        ...
        if self.terminal_dim <= 0:
            raise ValueError("terminal_dim 必须为正整数")
        ...
        # 末端位置映射：x_N = x_0 + C_pos_term * u
        pick_pos = np.hstack([np.eye(self.pos_dim), np.zeros((self.pos_dim, 3))])
        self.C_pos_term = self.dt * np.kron(np.ones((1, self.horizon)), pick_pos)
    ...
    def solve(
        ...
        *,
        ...
        terminal_state_jacobian: Optional[np.ndarray] = None,
    ) -> np.ndarray:
        ...
        if (
            ...
            and terminal_state_jacobian is not None
        ):
            # 新版：V(phi_N)，phi_N 由 feature state + 线性映射构成。
            terminal_state = np.asarray(terminal_state, dtype=float).reshape(-1)
            terminal_state_jacobian = np.asarray(terminal_state_jacobian, dtype=float)
            if terminal_state.shape[0] != self.terminal_dim:
                raise ValueError("terminal_state 维度与 terminal_dim 不一致")
            if terminal_state_jacobian.shape != (self.terminal_dim, self.task_dim * self.horizon):
                raise ValueError("terminal_state_jacobian 维度错误")

            phi0 = terminal_state
            phi_bar = phi0 + terminal_state_jacobian @ self.full_solution
            h_term, g_term, _ = self.terminal_value.quadratic_approx(phi_bar)
            H += terminal_state_jacobian.T @ h_term @ terminal_state_jacobian
            rhs += terminal_state_jacobian.T @ (h_term @ phi0 + g_term)
            term_phi0 = phi0
            term_jac = terminal_state_jacobian
            term_h = h_term
            term_g = g_term
            ...
            term_phi0 = e0
            term_jac = self.C_term
            term_h = h_term
            term_g = g_term

        self.full_solution = np.linalg.solve(H, rhs)
        u = self.full_solution
        track_res = self.A @ u - error_vector
        smooth_res = self.B @ u - u_anchor
        track_cost = 0.5 * float(track_res @ (self.G @ track_res))
        smooth_cost = 0.5 * float(smooth_res @ (self.Q @ smooth_res))
        terminal_cost = 0.0
        if (
            term_phi0 is not None
            and term_jac is not None
            and term_h is not None
            and term_g is not None
        ):
            phi = term_phi0 + term_jac @ u
            terminal_cost = 0.5 * float(phi @ (term_h @ phi)) + float(term_g @ phi)
        self.last_cost_terms = {
            "track": float(track_cost),
            "smooth": float(smooth_cost),
            "terminal": float(terminal_cost),
            "total": float(track_cost + smooth_cost + terminal_cost),
...
class MPCController:
    ...
    def __init__(
        ...
        terminal_value_model: Optional[TerminalValueModel] = None,
        ...
        self.arm_link_body_ids = np.array(
            sorted(set(int(self.model.jnt_bodyid[j]) for j in self.arm_joint_ids)),
            dtype=int,
        )
        ...
        if use_terminal_value:
            if terminal_value_model is not None:
                terminal_value = terminal_value_model
            else:
        ...
        if self.terminal_value_dim not in (3, 6, 7, 15):
            raise ValueError("terminal_value_dim 仅支持 3 / 6 / 7 / 15")
        ...
        self._terminal_feature_jacobian = None
        self._terminal_feature_scale = None
        if self.terminal_value_dim == 15:
            # 15D 特征缩放（用于数值归一化）:
            # [target(3), end(3), base_vel(3), q(6)]
            # target/end 以米为单位，范围已接近 O(1)，保持 1.0；
            # base_vel 和 q 做尺度归一化，避免网络偏向某几维。
            self._terminal_feature_scale = np.array(
                [
                    1.0, 1.0, 1.0,  # target
                    1.0, 1.0, 1.0,  # end
                    0.5, 0.5, 0.5,  # base_vel
                    np.pi, np.pi, np.pi, np.pi, np.pi, np.pi,  # q
                ],
                dtype=float,
            )
        if self.terminal_value_dim == 15:
            self._terminal_feature_jacobian = np.zeros(
                (15, self.mpc.task_dim * self.mpc.horizon), dtype=float
            )
            # 仅 end_N(第 4~6 维)依赖 MPC 控制量；其余 target/base_vel/q 视作局部常量。
            end_scale = self._terminal_feature_scale[3:6].reshape(3, 1)
            self._terminal_feature_jacobian[3:6, :] = self.mpc.C_pos_term / end_scale
    ...
    def _manipulability(self) -> float:
        jac_full = self._task_jacobian()
        jac = jac_full if self.use_orientation_task else jac_full[:3]
        gram = jac @ jac.T + 1e-9 * np.eye(jac.shape[0])
    ...
    def _terminal_feature_state(
        self, current_pos_world: np.ndarray, target_world: np.ndarray, terminal_abs_time: float
    ) -> np.ndarray:
        """
        15D 特征:
            [target_N(3), end_N_baseline(3), base_vel(3), q_current(6)]
        其中 end_N_baseline 为 x0（MPC 内部通过线性映射叠加控制增量）。
        """
        base_vel = self._base_velocity_world(terminal_abs_time)
        q_cur = self.data.qpos[self.arm_qpos_indices].copy()
        raw = np.concatenate([target_world.copy(), current_pos_world.copy(), base_vel, q_cur])
        if self._terminal_feature_scale is None:
            return raw
        return raw / self._terminal_feature_scale
    ...
    def _point_in_forbidden_zone(
        ...
        t = np.asarray(target_world, dtype=float).reshape(3)
        in_y = (t[1] - depth) <= p[1] <= t[1]
        out_center = (p[0] <= (t[0] - half_width)) or (p[0] >= (t[0] + half_width))
        return bool(in_y and out_center)

    def _arm_forbidden_occupancy(self, target_world: np.ndarray) -> int:
        count = 0
        for body_id in self.arm_link_body_ids:
            if self._point_in_forbidden_zone(self.data.xpos[body_id], target_world):
                count += 1
        return int(count)
    ...
    def _warm_start_to_pose(
        ...
        for k in range(steps):
            ...
            if k % self.control_every == 0:
                ...
                if self.terminal_value_dim == 15 and self._terminal_feature_jacobian is not None:
                    terminal_state = self._terminal_feature_state(
                        current_pos, future_targets[-1], terminal_time
                    )
                    twist = self.mpc.solve(
                        e_hat,
                        terminal_state=terminal_state,
                        terminal_state_jacobian=self._terminal_feature_jacobian,
                    )
                else:
        ...
        while True:
            ...
            if step_count % self.control_every == 0:
                ...
                if callback is not None:
                    u_seq = self.mpc.full_solution.reshape(self.mpc.horizon, self.mpc.task_dim)
                    u_lin = u_seq[:, :3]
                    pred_path = current_pos + self.control_dt * np.cumsum(u_lin, axis=0)
                    pred_terminal_pos = pred_path[-1]
                    pred_terminal_prev = pred_path[-2] if pred_path.shape[0] >= 2 else current_pos.copy()
                    terminal_err = self._terminal_error(
                        pred_terminal_pos, future_world[-1], terminal_time
                    )
                    if self.terminal_value_dim == 15:
                        terminal_state_log = self._terminal_feature_state(
                            pred_terminal_pos, future_world[-1], terminal_time
                        )
                    else:
                        terminal_state_log = terminal_err.copy()
                    qdot_limit_ratio = float(
                        np.max(np.abs(qdot_cmd) / np.maximum(self.velocity_limit, 1e-9))
                    )
                    qdot_raw_ratio = float(
                        np.max(np.abs(qdot_pre_clip) / np.maximum(self.velocity_limit, 1e-9))
                    )
                    target_terminal_prev = (
                        future_world[-2].copy() if future_world.shape[0] >= 2 else future_world[-1].copy()
                    )
                    pred_terminal_vel = (pred_terminal_pos - pred_terminal_prev) / self.control_dt
                    target_terminal_vel = (future_world[-1] - target_terminal_prev) / self.control_dt
                    terminal_speed_error = float(np.linalg.norm(pred_terminal_vel - target_terminal_vel))
                    terminal_dist = float(np.linalg.norm(pred_terminal_pos - future_world[-1]))
                    forbidden_count = self._arm_forbidden_occupancy(future_world[-1])
                    mpc_cost_terms = dict(getattr(self.mpc, "last_cost_terms", {}))
                    cb_payload = {
                        "time": float(sim_time),
                        "abs_time": float(self.data.time),
                        "current_pos": current_pos.copy(),
                        "target_pos": future_world[0].copy(),
                        "terminal_pos": pred_terminal_pos.copy(),
                        "terminal_target": future_world[-1].copy(),
                        "terminal_dist": terminal_dist,
                        "terminal_speed_error": terminal_speed_error,
                        "terminal_state": terminal_state_log.copy(),
                        "terminal_error": terminal_err.copy(),
                        "q": self.data.qpos[self.arm_qpos_indices].copy(),
                        "qdot": qdot_cmd.copy(),
                        "qdot_limit_ratio": qdot_limit_ratio,
                        "qdot_raw_ratio": qdot_raw_ratio,
                        "manipulability": float(self._manipulability()),
                        "singularity_metric": float(self._singularity_metric()),
                        "forbidden_count": int(forbidden_count),
                        "mpc_cost_track": float(mpc_cost_terms.get("track", 0.0)),
                        "mpc_cost_smooth": float(mpc_cost_terms.get("smooth", 0.0)),
                        "mpc_cost_terminal": float(mpc_cost_terms.get("terminal", 0.0)),
                        "mpc_cost_total": float(mpc_cost_terms.get("total", 0.0)),
                        "mpc_u_norm": float(mpc_cost_terms.get("u_norm", 0.0)),
                    }
                    cb_ret = callback(cb_payload)
                    should_stop = False
                    if isinstance(cb_ret, dict):
                        should_stop = bool(cb_ret.get("stop", False))
                    elif cb_ret is not None:
                        should_stop = bool(cb_ret)
                    if should_stop:
                        break
