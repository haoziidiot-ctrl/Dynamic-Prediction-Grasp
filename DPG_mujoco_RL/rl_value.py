...
说明:
    - 输入维度由训练模型决定（支持 3/6/7/15 等）。
    - 本项目推荐 15D 终端特征: [target_N, end_N, base_vel, q_current]。
"""
...
class TerminalValueModel:
    ...
    hessian_max: float = 80.0
    grad_clip: float = 120.0
    scale: float = 1.0
        ...
        if self._use_torch and self.model_path:
            try:
                ...
                if isinstance(payload, dict) and "state_dict" in payload:
                    ...
                    ckpt_dim = int(payload.get("input_dim", self.input_dim))
                else:
                    state_dict = payload
                    ckpt_dim = int(self.input_dim)
                self.input_dim = ckpt_dim
    ...
    def quadratic_approx(self, e: np.ndarray) -> Tuple[np.ndarray, np.ndarray, float]:
        ...
        if self.grad_clip > 0.0:
            ...
            if np.isfinite(g_norm) and g_norm > float(self.grad_clip):
                g *= float(self.grad_clip) / max(g_norm, 1e-9)
        return h, g, v
